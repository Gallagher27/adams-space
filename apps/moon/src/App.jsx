...
function AdminDialog({ items, blessings, onClose, onAdd, onDeleteItem, onDeleteBlessing, onDeleteAllBlessings, onLoadAccessVisits, remote, lang }) {
  ...
  return (
    <Modal title={unlocked ? (remote ? t(lang, "managementShared") : t(lang, "managementLocal")) : t(lang, "enterManagement")} onClose={onClose} wide={unlocked} busy={saving} lang={lang}>
      {!unlocked ? (
        ...
        <div className={`admin-layout admin-control-room ${remote ? "has-remote-log" : ""}`}>
          <header className="admin-hero admin-panel">
            <div className="admin-hero-mark" aria-hidden="true"><img src="/assets/art/blessing-star.png" alt="" /></div>
            <div className="admin-hero-copy"><p className="eyebrow">{t(lang, "adminRoomLabel")}</p><h3>{t(lang, "adminWorkspace")}</h3><p>{t(lang, "protectedNote")}</p></div>
            <span className={`admin-status-pill ${remote ? "is-connected" : "is-local"}`}><i aria-hidden="true" />{remote ? t(lang, "adminConnected") : t(lang, "adminLocalReady")}</span>
          </header>
          {remote && <div className="admin-password-card admin-panel"><div className="admin-card-head"><div><p className="eyebrow">{t(lang, "thisMonth")}</p><h3>{t(lang, "currentPassword")}</h3></div><span className="admin-card-index">01</span></div>{currentPassword ? <div className="admin-password-value"><code>{currentPassword}</code><span>{passwordUpdatedAt ? t(lang, "updatedAt", formatDate(passwordUpdatedAt, lang)) : t(lang, "tellFamily")}</span></div> : <button className="secondary-button" type="button" onClick={async () => { try { const result = await getRemotePassword(); setCurrentPassword(result.password); setPasswordUpdatedAt(result.updatedAt); } catch (error) { setError(localizedError(error, lang)); } }}>{t(lang, "viewPassword")}</button>}</div>}
          <form className="admin-form admin-panel form-stack timeline-composer" onSubmit={addItem}>
            ...
            <div className="admin-field-grid"><label>{t(lang, "title")}<input name="title" required placeholder={t(lang, "titlePlaceholder")} /></label><label>{t(lang, "occurredAt")}<input name="occurredAt" type="datetime-local" required defaultValue={formatDateTimeLocal()} /><span className="field-hint">{t(lang, "timeHint")}</span></label></div>
          ...
          {remote && <section className="access-log-panel" aria-labelledby="access-log-title">
            <div className="access-log-heading">
              <div className="access-log-heading-copy"><p className="eyebrow">{t(lang, "accessLogEyebrow")}</p><h3 id="access-log-title">{t(lang, "accessLogTitle")}</h3><p>{t(lang, "accessLogIntro")}</p></div>
              <div className="access-log-heading-actions"><span className="admin-card-index">03</span><button className="secondary-button access-log-refresh" type="button" onClick={refreshAccessVisits} disabled={accessVisitsLoading}>{accessVisitsLoading ? t(lang, "busy") : t(lang, "accessLogRefresh")}</button></div>
            </div>
