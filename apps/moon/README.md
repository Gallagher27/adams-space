...
- 时间线和家人祝福支持中英文内容；已有记录已补齐英文版本，未填写英文的新内容会在英文界面回退显示中文原文。

## 管理与共享

本地预览的管理入口使用本地管理密码，只用于离线演示。共享版由服务端访问密码保护，密码不写入前端；每月自动轮换，并保留登录、查看、上传与删除的审计记录。

共享版的时间线、留言和录音由 Cloudflare Pages Functions、D1 与 R2 保存，家人使用同一入口即可看到最新内容；发送者只能凭本机保存的撤回凭证删除自己的祝福，管理员使用单独的管理员密码后可以删除单条或清空全部访客祝福。录音只在用户明确点击并允许浏览器麦克风后开始。
